...
- 🖥️ Screensaver Mode (Autopilot): The snake plays itself using a
greedy pathfinding algorithm. Perfect for a terminal background.

- 🎮 Seamless Override: Press any arrow key to instantly switch from "Screensaver"
to "Manual" mode. Press a to switch back.

- ❄️ NixOS Themed: The snake head is a Lambda (λ), eating snowflakes (❄) and
packages (📦).

- 🚀 Performance: Built with pure crossterm for low-latency rendering and minimal
resource usage.
